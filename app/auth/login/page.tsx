...
function LoginForm() {
  ...
  const [mode, setMode] = useState<'email' | 'loading'>('email');
  ...
  async function handleGoogleLogin() {
    setMode('loading');
    // Store redirect intent in a cookie before leaving for OAuth
  ...
  if (mode === 'loading') {
    return (
      <div className="flex items-center justify-center min-h-[200px]">
        <div className="w-6 h-6 border-2 border-chayo-accent border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      <div className="mb-8">
        <h1 className="font-display text-3xl text-chayo-text mb-2">Welcome back.</h1>
        <p className="text-chayo-muted text-sm font-sans">
          Don&apos;t have an account?{' '}
          <Link href={`/auth/signup?next=${encodeURIComponent(next)}`} className="text-chayo-accent hover:underline">
            Create one
          </Link>
        </p>
      </div>
