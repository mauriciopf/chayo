...
export async function signInWithApple(): Promise<AuthUser> {
  ...
  // Note: Library automatically generates and SHA256-hashes the nonce
  // Note: Apple is sensitive to scope order - FULL_NAME must come first
  ...
  // Exchange with Supabase, passing the auto-generated nonce from the response
  const { data, error } = await supabase.auth.signInWithIdToken({
    provider: 'apple',
    token: credential.identityToken,
    nonce: credential.nonce, // Use the nonce generated by the library
