...
-- RLS Policies
CREATE POLICY "Users can view their organization's agent tools" ON agent_tools
  FOR SELECT USING (
    organization_id IN (
...
CREATE POLICY "Users can update their organization's agent tools" ON agent_tools
  FOR ALL USING (
    organization_id IN (
      SELECT tm.organization_id 
      FROM team_members tm 
